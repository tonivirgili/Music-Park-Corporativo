...
//conecta com o BD
	...
	$resultSet = $connection->prepare("SELECT * FROM CADASTROS WHERE LOCAL_='BC' ORDER BY SUBSTR( DATA_, 7, 4), SUBSTR( DATA_, 4, 2), SUBSTR( DATA_, 1, 2)");
...
//--------------------------------------------------------------------
	...
	$resultSet2 = $connection->prepare("SELECT `DATA_`, `NOME`, `EMAIL`, `TELEFONE`, `LOCAL_`, `TIPODEEVENTO`, `PESSOAS`, `MENSAGEM`, `ID` FROM CADASTROS WHERE LOCAL_='BC' ORDER BY SUBSTR( DATA_, 7, 4), SUBSTR( DATA_, 4, 2), SUBSTR( DATA_, 1, 2) LIMIT $start, $limit");
	// For Executing prepared statement we will use below function
    $resultSet2->execute();
